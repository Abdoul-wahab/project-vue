      ...
      <div v-for="artist in artists" :key="artist.id" @click="gotoArtiste(artist.id)" class="bg-gray-900 shadow-lg rounded p-3 cursor-pointer">
        <div class="group relative">
          ...
          <div class="absolute bg-black rounded bg-opacity-0 group-hover:bg-opacity-60 w-full h-full top-0 flex items-center group-hover:opacity-100 transition justify-evenly">
            <button @click.stop="like(artist.id)" class="hover:scale-110 text-white transform translate-y-3 group-hover:translate-y-0 transition opacity-0 group-hover:opacity-100 ring-0 focus:outline-none">
